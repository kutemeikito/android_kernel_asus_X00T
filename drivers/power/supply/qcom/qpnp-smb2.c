...
static int init_proc_charger_limit(void)
{
	...
	{
		pr_info("create proc entry %s success", CHARGER_LIMIT_EN_PROC_FILE);
		ret = 0;
	}
	...
	{
		pr_info("create proc entry %s success", CHARGER_LIMIT_PROC_FILE);
		ret = 0;
	}
	return ret;
}
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	pr_info("enter smb2_probe\n");
	if (!chip)
		return -ENOMEM;

#ifdef CONFIG_MACH_ASUS_X00T /* ASUS BSP allocate GPIO control */
	gpio_ctrl = devm_kzalloc(&pdev->dev, sizeof(*gpio_ctrl), GFP_KERNEL);
	pr_info("ADC_SW_EN=%d,ADCPWREN_PMI_GP1=%d\n",gpio_ctrl->ADC_SW_EN,gpio_ctrl->ADCPWREN_PMI_GP1);
	if (!gpio_ctrl)
		return -ENOMEM;
#endif
	...
	pr_info("enter1 smb2_probe HVDVP_reg=0x%x\n",HVDVP_reg);
...
#ifdef CONFIG_MACH_ASUS_X00T /* sw jeita per min in suspend */
